...
variable "app_cpu" {
  description = "App Runner vCPU (1024 = 1 vCPU). Each run spawns a short-lived node process that opens an in-memory DuckDB warehouse over the bundled fixtures, so 1 vCPU is ample."
  type        = string
  default     = "1024"
}
