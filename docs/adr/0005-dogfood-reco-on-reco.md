# ADR-0005 — Reco's GRC program runs on Reco

**Status:** proposed · **Date:** 2026-08-21 · **Needs:** product agreement on a Reco-on-Reco tenant

## Context

Reco's product discovers SaaS applications, maps identities and permissions across roughly 260
...
Reco already built it. The GRC program is currently not using it.
...
Point Reco at the Reco tenant and treat the Reco knowledge graph as a **collector** —
`mechanism: reco-graph` in the control record — sitting alongside the AWS, IdP and GitHub
...
**Explicit boundary, per ADR-0004:** Reco supplies observed state. Efficacy conclusions and
...
instrumenting Reco's own controls is a real customer-workflow gap found before a customer finds it.

**Bad.** Circularity to be honest about with the auditor. If Reco's product is the collection
mechanism for Reco's controls, a defect in the product is a defect in the evidence. The
mitigation is that the highest-consequence controls — cloud IAM, CI/CD, tenant isolation — are
collected from primary APIs independently of the product, and the reco-graph collector is
